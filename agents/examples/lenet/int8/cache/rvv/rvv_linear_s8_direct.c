    ...
    for (int m = 0; m < M; m++) {
        for (int n = 0; n < N; n++) {
            ...
            for (; k + 2*vlmax <= K; k += 2*vlmax) {
                ...
                vint8m1_t va0 = __riscv_vle8_v_i8m1(in_row + k, vl);
                vint8m1_t vb0 = __riscv_vle8_v_i8m1(w_row + k, vl);
                vint16m2_t prod0 = __riscv_vwmul_vv_i16m2(va0, vb0, vl);
                vacc = __riscv_vwadd_wv_i32m4(vacc, prod0, vl);

                vint8m1_t va1 = __riscv_vle8_v_i8m1(in_row + k + vlmax, vl);
                vint8m1_t vb1 = __riscv_vle8_v_i8m1(w_row + k + vlmax, vl);
                vint16m2_t prod1 = __riscv_vwmul_vv_i16m2(va1, vb1, vl);
                vacc = __riscv_vwadd_wv_i32m4(vacc, prod1, vl);
            }
            for (; k < K; k += vlmax) {
                size_t vl = __riscv_vsetvl_e8m1(K - k);
