        ...
        for (int oc = 0; oc < OC; oc++) {
            ...
            for (int row = 0; row < M; row++) {
                ...
                for (int k = 0; k < K; k += vl) {
                    ...
                    vfloat32m1_t va = __riscv_vle32_v_f32m1(&im2col_buf[row * K + k], vl);
                    vfloat32m1_t vw = __riscv_vle32_v_f32m1(&weight[oc * K + k], vl);
                    vacc = __riscv_vfmacc_vv_f32m1(vacc, va, vw, vl);
                }
                vfloat32m1_t vinit = __riscv_vfmv_s_f_f32m1(0.0f, 1);
                vfloat32m1_t vsum = __riscv_vfredusum_vs_f32m1_f32m1(vacc, vinit, vlmax);
