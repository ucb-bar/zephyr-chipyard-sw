                ...
                for (int ow = 0; ow < OW; ow += vl) {
                    ...
                    for (int kh = 0; kh < KH; kh++) {
                        ...
                        for (int kw = 0; kw < KW; kw++) {
                            int iw = iw0 + kw;
                            vfloat32m4_t v_input = __riscv_vlse32_v_f32m4(
                                input + ((n*C + c)*IH + ih)*IW + iw,
                                (ptrdiff_t)sizeof(float) * SW, vl);
                            v_max = __riscv_vfmax_vv_f32m4(v_max, v_input, vl);
                        }
                    }
                    __riscv_vse32_v_f32m4(output + ((n*C + c)*OH + oh)*OW + ow, v_max, vl);
                }
            }
        }
    }
