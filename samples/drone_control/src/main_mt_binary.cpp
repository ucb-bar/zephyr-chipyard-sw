...
#include <zephyr/kernel.h>
#include <zephyr/device.h>
#ifndef CONFIG_BOARD_SPIKE_RISCV64
#include <zephyr/drivers/uart.h>
#endif
#include <zephyr/timing/timing.h>
#include <zephyr/sys/atomic.h>
...
#ifndef CONFIG_BOARD_SPIKE_RISCV64
    k_mutex_lock(&uart_mutex, K_FOREVER);
    for (int i = 0; i < 4; i++) uart_poll_out(uart0, HEADER[i]);
    uart_poll_out(uart0, id);
    for (int j = 0; j < NACTIONS; j++) {
        uint8_t *pb = (uint8_t *)&u[j];
        for (int b = 0; b < (int)sizeof(float); b++) uart_poll_out(uart0, pb[b]);
    }
    uint8_t *ns_bytes = (uint8_t *)&ns;
    for (int b = 0; b < (int)sizeof(ns); b++) uart_poll_out(uart0, ns_bytes[b]);
    k_mutex_unlock(&uart_mutex);
#else
    printk("id=%d ns=%u u=[%.3f %.3f %.3f %.3f]\n",
           id, ns, (double)u[0], (double)u[1], (double)u[2], (double)u[3]);
#endif
}
...
#else
    float dummy_state[NSTATES] = {0};
    for (int i = 0; i < NUM_DRONES; i++) {
        memcpy(drone_tasks[i].obs, dummy_state, sizeof(dummy_state));
        atomic_set(&drone_tasks[i].new_state, 1);
    }
    k_sleep(K_MSEC(5000));
#endif
    return 0;
