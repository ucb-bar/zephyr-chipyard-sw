...
def plot_full_trace(wall_times, powers, output_path):
    ...
    plt.title('Power Trace - Full Sample', fontsize=14)
...
def plot_window_trace(wall_times, powers, start_time, end_time, output_path):
    ...
    plt.xlabel('Time (s) relative to measurement window start', fontsize=12)
    plt.ylabel('Power (W)', fontsize=12)
    plt.title(f'Power Trace - Measurement Window\n(Benchmark: {start_time:.6f} to {end_time:.6f})', fontsize=14)
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_path, dpi=150)
...
def create_heatmap(scalar_data, opu_data, output_path):
    ...
    ax1.set_title('Scalar Average Power (W)', fontsize=14)
    ax1.set_xlabel('Output Channels', fontsize=12)
    ax1.set_ylabel('Input Channels', fontsize=12)
    ...
    ax2.set_title('OPU (Vector) Average Power (W)', fontsize=14)
    ax2.set_xlabel('Output Channels', fontsize=12)
    ax2.set_ylabel('Input Channels', fontsize=12)
...
def create_perf_heatmaps(scalar_perf, opu_perf, output_dir):
    ...
    def _style_axis(ax, title, cbar_label):
        ...
        ax.set_xlabel('Output Channels', fontsize=12)
        ax.set_ylabel('Input Channels', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'perf_opu_heatmap.png'), dpi=150)
    plt.close(fig)
